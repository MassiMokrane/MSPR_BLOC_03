...
df = pd.read_excel("C:/Users/Massi/Desktop/MSPR BLOC 03/DATA_Set_brut/2017_tour1.xlsx", sheet_name="Commune", header=None)

# Afficher les 20 premières lignes pour repérer où commence l'en-tête
print(df.head(20))
# === 2. Détecter automatiquement la ligne d'en-tête ===
df = pd.read_excel("C:/Users/Massi/Desktop/MSPR BLOC 03/DATA_Set_brut/2017_tour1.xlsx", sheet_name="Commune", header=0)
...
df["Année"] = 2017  # Changez en 2017 pour l'autre dataset
...
# === 9. Orientation politique & score (simplifiée sur échelle de 0 à 4) ===
orientation_blocs = {
    "Extrême gauche": ["LFI", "LO", "NPA"],
    "Gauche": ["PCF", "Génération.s", "EELV", "PS"],
    "Centre": ["Renaissance", "MoDem", "Horizons", "Résistons"],
    "Droite": ["LR", "DLF", "UPR"],
    "Extrême droite": ["RN", "Reconquête"]
}

# Nouvelle échelle simplifiée
echelle_orientation = {
    "Extrême gauche": 0,
    "Gauche": 1,
    "Centre": 2,
...
def get_bloc_orientation(parti):
    if pd.isna(parti) or parti == "Inconnu":
        return ("Inconnu", None)
    for orientation, partis in orientation_blocs.items():
        if parti in partis:
            return (orientation, echelle_orientation[orientation])
    return ("Divers", None)

# Appliquer la nouvelle classification
df[["Orientation Politique", "Score Orientation (0 à 4)"]] = df["Parti Politique Élu"].apply(
    lambda p: pd.Series(get_bloc_orientation(p))
)
...
df_nom_fichier_excel = "DATA_Nettoyer_t1_2017.xlsx"
sauvegarder_fichier_excel(df, df_nom_fichier_excel)

# Sauvegarder en CSV
df_nom_fichier_csv = "DATA_Nettoyer_t1_2017.csv"
